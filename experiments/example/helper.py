...
def compute_stddev(units):
    return 1.0 / math.sqrt(float(units))
...
def read_data_sets(validation_size=5000, seed=None):
    ...
    str_train_vectors = pd.read_csv("datasets\\train_vectors.txt", header = None)
    str_train_labels = pd.read_csv("datasets\\train_labels.txt", header = None)
    ...
    train_labels = train_labels[0][validation_size:]
